#!/usr/bin/env python3
"""
MiValta Qwen2.5-1.5B — GGUF Conversion & Quantization

Converts the fine-tuned (merged) Qwen2.5-1.5B-Instruct model to GGUF Q4_K_M
...
Target: ~0.9-1.0 GB Q4_K_M file — significantly smaller than Gemma 3n E2B (~2.8 GB).

Usage:
    # Convert merged model to GGUF Q4_K_M (default)
    python convert_qwen.py --model_path ./models/josi-qwen-merged

    # Multiple quantization levels
    python convert_qwen.py --model_path ./models/josi-qwen-merged --quant q4_k_m q5_k_m

    # Keep F16 intermediate
    python convert_qwen.py --model_path ./models/josi-qwen-merged --keep_fp16

    # List quantization options
    python convert_qwen.py --list_quant
...
# Quantization options with Qwen2.5-1.5B size estimates
QUANT_LEVELS = {
    "f16":    {"bits": 16, "size_est": "~3.1 GB",  "quality": "highest",    "mobile": False},
    "q8_0":   {"bits": 8,  "size_est": "~1.6 GB",  "quality": "excellent",  "mobile": True},
    "q5_k_m": {"bits": 5,  "size_est": "~1.1 GB",  "quality": "very good",  "mobile": True},
    "q4_k_m": {"bits": 4,  "size_est": "~0.9 GB",  "quality": "good",       "mobile": True},   # RECOMMENDED
    "q4_0":   {"bits": 4,  "size_est": "~0.9 GB",  "quality": "acceptable", "mobile": True},
    "q3_k_m": {"bits": 3,  "size_est": "~0.7 GB",  "quality": "lower",      "mobile": True},
    "q2_k":   {"bits": 2,  "size_est": "~0.6 GB",  "quality": "lowest",     "mobile": True},
}
...
def _patch_llama_pretokenizer(llama_cpp_path: Path, token_hash: str):
    ...
    patch_lines = [
        ...
        f'{indent}    # qwen2.5 fine-tuned tokenizer (auto-patched by convert_qwen.py)\n',
...
def convert_to_gguf(model_path: str, output_path: str, llama_cpp_path: str = None):
    """Convert HuggingFace Qwen model to GGUF format (fp16).
    ...
    # Qwen2.5 uses BPE natively (tokenizer.json) — no SentencePiece tokenizer.model needed.
    # This is simpler than Gemma 3n which requires tokenizer.model for correct conversion.

    print(f"Converting {model_path} to GGUF (Qwen2 architecture)...")
...
def full_pipeline(
    ...
    """Run full conversion: HF Qwen → GGUF F16 → Quantized."""
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"  Qwen2.5-1.5B GGUF Export Complete")
    print(f"{'='*60}")
    print(f"\n  Model: {BASE_MODEL_ID}")
    ...
    print(f"\n  Next steps:")
    print(f"    1. Evaluate: python evaluate_qwen.py --model {list(results.values())[0]}")
...
def print_quant_options():
    ...
    print(f"\nQwen2.5-1.5B-Instruct ({BASE_MODEL_ID}) — Quantization Options")
    print("-" * 80)
    ...
    print(f"  q4_k_m (~0.9 GB) — best quality/size balance for 1.5B model")
    print(f"  q5_k_m (~1.1 GB) — slightly better quality, still very mobile-friendly")
    print(f"  q8_0   (~1.6 GB) — excellent quality if storage allows")


def main():
    parser = argparse.ArgumentParser(
        description="Convert fine-tuned Qwen2.5-1.5B to GGUF for on-device deployment"
    )
