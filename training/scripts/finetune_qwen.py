#!/usr/bin/env python3
"""
MiValta Josi — Qwen2.5-1.5B-Instruct LoRA Fine-Tuning Script

Fine-tunes Qwen/Qwen2.5-1.5B-Instruct (1.54B params, 1.31B non-embedding)
on Josi coaching training data (LLMIntent JSON output format).

Architecture notes:
  - Qwen2.5 uses AutoModelForCausalLM + AutoTokenizer (standard HF pipeline)
  - Native system role in chat template (no workarounds needed)
  - Standard ChatML-style messages: {"role": "system/user/assistant", "content": "text"}
  - GQA: 12 Q heads, 2 KV heads — efficient inference
  - bf16 loading (~3 GB) + LoRA adapters + gradient checkpointing (fits ~6 GB VRAM)
  - 4-bit QLoRA also works (no AltUp issues like Gemma 3n), but bf16 is fine at 1.5B
  - LoRA targets: attention + MLP projections
  - Merged + GGUF Q4_K_M for on-device deployment (~1 GB)
...
Usage:
    # Train interpreter (default)
    python finetune_qwen.py train

    # Train explainer
    python finetune_qwen.py train --mode explainer

    # Train with custom params
    python finetune_qwen.py train --lr 3e-5 --epochs 4

    # Merge LoRA weights into base model
    python finetune_qwen.py merge --lora_path ./models/josi-qwen-*/lora_weights

    # Quick sanity check on merged model
    python finetune_qwen.py sanity --model_path ./models/josi-qwen-merged

Requirements:
    pip install -U transformers>=4.37.0 torch peft datasets accelerate trl
"""
...
# Local path (pre-downloaded, falls back to HF hub)
...
# LoRA targets on the language model backbone (same projections as Gemma, Qwen2.5 uses the same names)
LORA_TARGET_MODULES = [
...
BATCH_SIZE = 2             # Qwen 1.5B in bf16 is ~3 GB — fits larger micro-batch
...
# =============================================================================
# QWEN 2.5 MESSAGE FORMAT
# =============================================================================
# Qwen2.5-Instruct uses standard ChatML-style messages natively:
#   {"role": "system",    "content": "plain text"}
#   {"role": "user",      "content": "plain text"}
#   {"role": "assistant", "content": "plain text"}
#
# No conversion needed — our training data is already in this format.
# =============================================================================


# =============================================================================
# JOSI SYSTEM PROMPTS — Dual-Model Architecture
# =============================================================================
...
def prepare_dataset(path: str, tokenizer, max_seq_length: int = 1024) -> Dataset:
    ...
    Qwen2.5 uses this format natively — no conversion needed.
...
def load_model_and_tokenizer(model_id: str = None):
    ...
    At 1.54B params, bf16 is ~3 GB — fits comfortably on any modern GPU.
    """
    if model_id is None:
    ...
    # of that token ID to -100 in labels — including the real stop token at
    ...
    print(f"  1.54B params (1.31B non-embedding)")
    print(f"  GQA: 12 Q heads, 2 KV heads")
    print(f"  Loading in bf16 (~3 GB VRAM)")
...
def train(
    ...
    """Run LoRA fine-tuning on Qwen2.5-1.5B-Instruct.
    ...
    if output_dir is None:
        ...
        output_dir = f"./models/josi-qwen-{mode}-{timestamp}"
    ...
    if use_wandb:
        try:
            import wandb
            wandb.init(
                project="mivalta-josi-qwen",
                name=f"josi-qwen-{mode}-{datetime.now().strftime('%m%d_%H%M')}",
                config=run_config,
            )
            report_to = "wandb"
    ...
    # Train
    print("\n" + "=" * 60)
    print(f"Starting Josi training — Qwen2.5-1.5B-Instruct (bf16 + LoRA)")
    print(f"  Mode: {mode.upper()} ({'GATCRequest JSON' if mode == 'interpreter' else 'plain coaching text'})")
    print(f"  Model: {MODEL_ID}")
    print(f"  Architecture: Qwen2ForCausalLM")
    print(f"  Params: 1.54B total, 1.31B non-embedding")
...
def merge(
    ...
    """Merge LoRA weights with base Qwen2.5-1.5B-Instruct model for GGUF export.
    ...
    print(f"  python convert_qwen.py --model_path {output_path}")
...
def sanity_check(model_path: str, max_new_tokens: int = 200, mode: str = "both"):
    ...
    for i, prompt in enumerate(prompts):
        ...
        messages = [
            {"role": "system", "content": build_system_prompt(prompt["mode"])},
...
def main():
    parser = argparse.ArgumentParser(
        description="MiValta Josi — Qwen2.5-1.5B-Instruct LoRA Fine-Tuning",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Train interpreter (GATCRequest JSON output)
  python finetune_qwen.py train --mode interpreter
  python finetune_qwen.py train --mode interpreter --lr 3e-5 --epochs 4

  # Train explainer (plain coaching text output)
  python finetune_qwen.py train --mode explainer
  python finetune_qwen.py train --mode explainer --no-wandb

  # Merge LoRA weights
  python finetune_qwen.py merge --lora_path ./models/josi-qwen-interpreter-*/lora_weights

  # Sanity check
  python finetune_qwen.py sanity --model_path ./models/merged --mode both
        """,
    )
