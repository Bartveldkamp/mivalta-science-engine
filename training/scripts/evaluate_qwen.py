#!/usr/bin/env python3
"""
Qwen2.5-1.5B Evaluation Script — Dual-Mode Architecture

Evaluates the fine-tuned Qwen2.5-1.5B-Instruct model in two separate modes:
...
Usage:
    ...
    python evaluate_qwen.py \
        --interpreter models/josi-qwen-interpreter-*/final \
        --explainer models/josi-qwen-explainer-*/final --verbose

    # Evaluate a single mode with a HuggingFace model
    python evaluate_qwen.py --hf-model ./models/merged --mode explainer --verbose

    # Evaluate with GGUF
    python evaluate_qwen.py --model path/to/model.gguf --mode interpreter --verbose
"""
...
def _parse_gguf_output(raw: str) -> str:
    """Parse GGUF output for Qwen2.5 chat template format.

    Qwen2.5 uses <|im_start|>/<|im_end|> tokens in its chat template.
    """
    ...
    if "<|im_start|>assistant" in output:
        ...
        response = parts[-1]
        # Strip the newline after the role marker
        if response.startswith("\n"):
            response = response[1:]
        response = response.split("<|im_end|>")[0]
        response = response.split("<|im_start|>")[0]
    # Fallback: Gemma-style markers (in case of mixed templates)
    elif "<start_of_turn>model" in output:
...
def run_prompt_gguf(model_path: str, prompt: str, system_prompt: str, llama_cli: str, max_tokens: int = 150) -> str:
    """Run a prompt through a GGUF model using llama-cli.

    Uses Qwen2.5 ChatML format: <|im_start|>role\ncontent<|im_end|>
    """
    import tempfile, os
    ...
    prompt_file = None
    try:
        fd, prompt_file = tempfile.mkstemp(suffix='.txt', prefix='josi_q_')
...
def run_prompt_hf(model, tokenizer, prompt: str, system_prompt: str, device: str, max_tokens: int = 150) -> str:
    """Run a prompt through a HuggingFace Qwen model.

    Uses standard ChatML messages — Qwen2.5 handles this natively.
    """
    import torch
...
def run_explainer_eval(run_fn, label: str, verbose: bool = False) -> dict:
    ...
    print(f"  Model family: Qwen2.5-1.5B-Instruct")
    print(f"{'=' * 60}\n")
...
LOCAL_BASE_PATH = Path(__file__).resolve().parent.parent / "models" / "Qwen2.5-1.5B-Instruct"
...
def load_hf_model(model_name: str):
    """Load a Qwen2.5 model for evaluation.

    Supports:
    - LoRA adapter checkpoints (auto-merges with base model)
    - Full merged models
    """
    import torch
    ...
    if _is_lora_checkpoint(model_name):
        ...
        if device == "cuda":
            ...
            base_model = AutoModelForCausalLM.from_pretrained(
                base_id, torch_dtype=torch.float32, trust_remote_code=False,
            )
        ...
        for candidate in [model_name, str(model_path.parent / "lora_weights"), base_id]:
            try:
                tokenizer = AutoTokenizer.from_pretrained(candidate, trust_remote_code=False)
                break
            except (OSError, ValueError):
                continue
        if tokenizer is None:
            raise RuntimeError(f"Cannot load tokenizer from {model_name} or base model {base_id}")
    else:
        ...
        if device == "cuda":
            ...
            model = AutoModelForCausalLM.from_pretrained(
                model_name, torch_dtype=torch.float32, trust_remote_code=False,
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Qwen2.5-1.5B — Dual-Mode (Interpreter + Explainer)"
    )
    ...
    if has_dual:
        if args.interpreter:
            model, tokenizer, device = load_hf_model(args.interpreter)
            label = args.interpreter
            run_fn = lambda prompt: run_prompt_hf(model, tokenizer, prompt, interp_prompt, device, max_tokens=100)
        ...
        if args.explainer:
            model, tokenizer, device = load_hf_model(args.explainer)
            label = args.explainer
            run_fn = lambda prompt: run_prompt_hf(model, tokenizer, prompt, expl_prompt, device, max_tokens=150)
